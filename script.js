...
- [어휘 뜻 문항] 영단어 1개(또는 숙어)의 한글 의미를 묻는 문항의 경우, 영어사전에 등재된 모든 의미가 정답. 모범답안에 없더라도 사전적으로 올바른 의미라면 반드시 정답으로 인정
                ...
                <div class="flex items-center gap-4 w-full">
                    ...
                    <div class="flex items-center gap-2 ml-4">
                        ...
                        <button id="ai-tab-done"
                            class="btn-ys !bg-white !text-slate-500 !border-2 !border-slate-300 hover:!border-[#013976] hover:!text-[#013976] !px-5 !py-2.5 !text-[15px] !font-black rounded-xl whitespace-nowrap flex items-center gap-2" style="width:150px; justify-content:center;" onclick="onAITabDoneClick()">✅ AI 채점완료자</button>
                    </div>
                </div>
            </div>
...
function onAITabDoneClick() {
    if (window._aiGradeTemp && Object.keys(window._aiGradeTemp).length > 0) {
        if (!confirm('채점 완료 후 확인 버튼을 누르지 않은 학생이 있습니다.\n탭을 이동하면 채점 결과가 저장되지 않습니다.\n계속하시겠습니까?')) return;
    }
    switchAIGradeTab('done');
}
...
function getGradeTone(grade) {
    ...
    const HONORIFIC = '\n[필수 규칙] 모든 문장은 반드시 ~ㅂ니다/~습니다 형식의 격식 존댓말로 작성하세요. ~요, ~네요, ~거예요 등 해요체는 절대 사용하지 마세요. 반말도 절대 금지입니다. 문단 사이에 빈 줄을 넣지 마세요.\n' +
        '⚠️ [간결한 서술어 필수]: 문장을 끝맺을 때 "~단계에 해당합니다", "~함을 확인했습니다", "~상태를 나타내고 있습니다" 같이 쓸데없이 길고 번잡한(장황한) 종결어미는 절대 사용하지 마세요. 대신 "~부족합니다", "~필요합니다", "~시급합니다", "~권장합니다" 처럼 군더더기 없이 가장 간결하고 명확한 서술어로 깔끔하게 끝맺으세요.\n' +
...
[톤앤매너] 따뜻하고 친근한 말투로 작성하세요. 어려운 용어는 쓰지 마세요. 칭찬을 먼저 충분히 하고, 개선점은 "다음엔 이렇게 해보면 어떨까요?" 같이 부드럽게 제안하세요. 항상 격려로 마무리하세요.
⛔ 금지 표현 (절대 사용 금지): "부진", "미흡", "부족", "낮다" 같은 직접적인 부정 표현 절대 금지. 대신 "조금 더 노력이 필요합니다", "잘 따라오고 있습니다" 처럼 학생이 상처받지 않는 부드러운 표현만 사용하세요.${HONORIFIC}`;
    }
    // 고등: 고1~3
...
function _pctLabel(pct, prefix = '전체') {
    ...
    if (p <= 10) return `${prefix} 최상위권`;
    if (p <= 20) return `${prefix} 상위권`;
...
${diffAnalysis}
- 제시된 [문항 난이도별 오답 현황] 데이터를 분석하여, 학생이 현재 어느 난이도 수준의 문제까지 소화하고 있으며 어느 난이도부터 오답이 발생하는지(취약한지) 객관적으로 짚어주는 문장을 코멘트에 반드시 포함하세요. (작성 예시 1: "기초부터 중간(중) 난이도의 문항까지는 무난하게 소화하고 있으나, 상 난이도부터 오답이 발생하여 심화 학습이 요구됩니다." / 작성 예시 2: "중 난이도 문제를 해결할 기본기는 갖추었으나, 오히려 쉬운 하 난이도에서 오답이 발생하여 꼼꼼한 문제 풀이 습관이 필요합니다.")
...
- 실제 총점, 만점, 성취수준, 백분위 지표들을 서술할 때는 장황하게 늘어놓지 말고, 반드시 "${totalMax}점 만점에 ${totalScore}점을 기록(전체 백분위 약 ${oaUpperPercentile}%${clsTotalPercentile !== null ? ', ' + classLabelText + ' 내 백분위 약 ' + clsTotalPercentile + '%' : ''})하여 성취 수준이 매우 우수합니다(또는 부족합니다)." 와 같이 제공된 형식 그대로 불필요한 수식어 없이 단 한 문장으로 간결하게 종결하세요. 호칭이 필요하면 "${sName} 학생은" 형식만 사용하세요.
...
- 영역명을 영어(Grammar, Reading 등)로 쓰지 마세요. 한국어(문법, 독해 등)로만 쓰세요.
- 학생을 묘사할 때 경어(-시- 존칭: 받으신, 획득하신, 기록하셨으므로 등) 절대 사용 금지. "획득하여", "기록했으므로" 형식으로 쓰세요.
- 실제 점수, 만점, 성취수준, 전체 백분위를 서술할 때는 장황하게 늘어놓지 말고, 반드시 "${maxScore > 0 ? maxScore : '?'}점 만점에 ${studentScore}점을 기록(전체 백분위 약 ${upperPercentile}%)하여 성취 수준이 매우 우수합니다(또는 부족합니다)." 와 같이 제공된 형식 그대로 불필요한 수식어 없이 단 한 문장으로 간결하게 종결하세요. 호칭이 필요하면 "${sName} 학생은" 형식만 사용하세요.
- 학원명, 교재명, 브랜드명 절대 금지. 모든 답변은 순수 한국어로 작성하세요.
- ⛔ "수업을 잘 따라오고 있습니다", "수업에 적응하고 있습니다", "학원 생활" 등 재원생 대상 표현 절대 금지. (이 시험은 입학 전 레벨테스트임)
- ⛔ 줄바꿈(\n, 개행) 절대 금지. 전체 코멘트를 하나의 연속된 문단으로 작성하세요. 마침표(.), 물음표(?), 느낌표(!) 뒤에는 반드시 띄어쓰기(공백)를 한 칸 포함하여 문장을 이어 나가세요.
- 성취레벨 단어(예: 매우 우수, 많이 부족 등)를 코멘트에 서술할 때는 '성취레벨 많이 부족'처럼 기계적 용어를 그대로 꽂아 넣어 딱딱하고 부자연스러운 문장을 절대 만들지 마세요. 대신 '성취도가 많이 부족하여 기초를 다질 필요가 있습니다' 또는 '성취 수준이 많이 부족한 단계에 해당합니다'처럼 학부모에게 보내는 정성스럽고 부드러운 교사의 언어로 자연스럽게 다듬어 작성하세요.
